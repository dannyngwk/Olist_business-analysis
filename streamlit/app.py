...
# ---- Sidebar filters ----
states = q("SELECT DISTINCT customer_state FROM analytics_marts.fct_orders "
...
FROM analytics_marts.fct_orders {state_clause}""").iloc[0]
...
with left:
    ...
    m = q("""SELECT order_month, gmv FROM analytics_marts.fct_monthly_revenue
...
with right:
    ...
    d = q("""SELECT CASE WHEN is_late THEN 'Late' ELSE 'On-time' END status,
             avg(review_score) score FROM analytics_marts.fct_orders
...
with left:
    ...
    cat = q("""SELECT category, revenue, avg_review_score
              FROM analytics_marts.fct_category_performance ORDER BY revenue DESC LIMIT 10""")
...
with right:
    ...
    g = q(f"""SELECT customer_state, sum(gmv) gmv FROM analytics_marts.fct_orders
...
rep = q("SELECT 100.0*sum(CASE WHEN is_repeat THEN 1 ELSE 0 END)/count(*) pct "
        "FROM analytics_marts.dim_customers").iloc[0].pct
