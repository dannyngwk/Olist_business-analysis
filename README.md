...
│   ├── meltano.yml          # taps, targets, CSV → stream mapping, env config
│   └── plugins/             # committed plugin lockfiles (reproducible installs)
...
referential integrity) plus freshness checks. CI validates that the full dbt
DAG and the Dagster definitions compile on every push, with no cloud
credentials required.

Two load-correctness guards are baked into `meltano/meltano.yml`:
- `product_category_name_translation.csv` ships with a UTF-8 BOM, so the
  tap reads it with `encoding: utf-8-sig`.
- The DuckDB loader pins `duckdb==1.0.0`. Newer DuckDB auto-detects a header
  during `COPY` and silently drops the first row of every loaded table.
